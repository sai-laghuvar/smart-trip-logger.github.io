...
// Very simple structured parser supporting "key: value" comma-separated pairs.
...
export const ingest = internalMutation({
  args: {
    from: v.string(), // WhatsApp sender number like "whatsapp:+15551234567" or "+15551234567"
    body: v.string(),
  },
  handler: async (ctx, args) => {
    ...
    const parsed = parseStructured(args.body);
    ...
    // Normalize "from" to a bare E.164 phone number (strip "whatsapp:")
    const normalizedFrom = args.from.replace(/^whatsapp:/i, "").trim();
    const emailAlias = `${normalizedFrom.replace(/[^+\d]/g, "")}@wa.local`;

    // Find or create a lightweight user for this phone
    ...
    const userId =
      ...
      (await ctx.db.insert("users", {
        email: emailAlias,
        name: `WhatsApp User ${normalizedFrom}`,
        image: undefined,
    ...
    const summary = `Trip logged: ${trip.origin} → ${trip.destination} via ${trip.transportMode} at ${trip.time} on ${trip.date}${trip.coTravelers ? ` with ${trip.coTravelers}` : ""}`;
    return summary;
  },
});
